...
public class EnvironmentManager : MonoBehaviour
{
    ...
    public void Start()
    {
        ...
        //10 Read Image from resources
        _sourceImage = Resources.Load<Texture2D>("Data/SourceField");
        
        //11 Create grid from image
        _voxelGrid = new VoxelGrid(_sourceImage, 10, transform.position, 1f);

        //31 Read image and set states of GraphVoxels
    ...
    //39 Create public method to change the visibility of the void voxels
    /// <summary>
    /// Change the visibility of the void GraphVoxels
    /// </summary>
    public void VoidsVisibility()
    {
        //40 Iterate through every voxel
        foreach (GraphVoxel voxel in _voxelGrid.Voxels)
        {
            //41 FIRST Check if voxel is active and not not an obstacle
            //61 SECOND Also check if voxel isn't target before toggling visibility
            //83 THIRD Also check if voxel isn't part of path
            if (voxel.IsActive && !voxel.IsObstacle && !voxel.IsTarget && !voxel.IsPath)
            {
                //43 Implement the Toggle Visibility method
